...
def test_rounding_dust_does_not_stop_the_run():
    """Nordnets 4-decimalers avrundning vid 'sälj allt' får inte larma vakten."""
    portfolio_metadata = pd.DataFrame(
        [
            {
                "Portfolio_Name": "EGEN",
    ...
    # Köp 5.0, sälj 5.0001 → ackumulerat innehav -0.0001 (rundningsdamm), inte
    # en verklig saknad ingående balans. Vakten ska inte stoppa körningen.
    transactions = pd.DataFrame(
        [
            {
                "Portfolio_Name": "EGEN",
            ...
            {
                "Portfolio_Name": "EGEN",
                COL_AFFARSDAG: pd.Timestamp("2024-01-03"),
                "ISIN": "SE0001",
                "Antal": 5.0001,
                "Transaktionstyp": SELL,
                "Belopp": 600.0,
    ...
    out = build_portfolios_and_benchmarks(inputs)
    assert "PORT_EGEN_REAL" in out
